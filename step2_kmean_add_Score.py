...
def 单个商户_单个正面因子_score(row,column):
    商户名=df_total.ix[row,0]
    print("商户名:",商户名)
    正面因子名=list_goodNames[column]
    print("正面因子名:",正面因子名)
    正面因子=df_total.ix[row,column+1]
    print("正面因子:",正面因子)
    此因子中心点=df_goodCenters[正面因子名]
    差值=此因子中心点-正面因子
    差值_最小值=min(abs(差值))
    print("差值_最小值:",差值_最小值)
    #返回差值中最小值的索引index
...
#单个商户（整形医院）所有正面因子分数计算，row行的索引，column列的索引
def 单个商户_正面因子总分(row): 
    total_score=0
    商户名=df_total.ix[row,0]
    for column in range(len(list_goodNames)):
        score=单个商户_单个正面因子_score(row,column)
        total_score+=score
        column+=1
    print("商户名:",商户名)
    print("所有正面因子分数:",total_score)
    return total_score
...
def 所有商户_正面因子总分():
    ...
    for row in range(所有商户数量):
        商户名=df_total.ix[row,0]
        score=单个商户_正面因子总分(row)
        dict_商户_正面因子总分[商户名]=score
    return dict_商户_正面因子总分

#存储商户名和各自的正面因子总分
dict_商户_正面因子总分={}
dict_商户_正面因子总分=所有商户_正面因子总分()
  

#商户名列表
b=list(dict_商户_正面因子总分)
#商户名总分
c=list(dict_商户_正面因子总分.values())

data=pd.DataFrame({"商户名":b,"正面因子总分":c})  
data.to_excel("result_商户正面因子总分.xlsx")
